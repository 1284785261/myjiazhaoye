                ...
                <td class="boxs">
                  <div class="ivu-main-img">
                    <div class="item-img">
                      ...
                      <div class="uplodas" v-for="(item,index) in labelList2" v-if="radio4 == '0'">
                        <div  v-if="!imgList[index]" @click="indexs = index">
                          <input type="file" accept="image/*" name="file" class="file" @change="uploadfile" />
                          <Icon type="camera" class="icons"></Icon>
                          <span class="titew">上传{{item}}</span>
...
<script>
  ...
  import { hostOfficeList, hostSignOffice, hostRoomUser, hostWay, hostController,host,OfficeContractSign200279,CxkjCommunityContractSignReletInfo200259,imgPath,Registeruser200222,ContractSignInfo200225 } from '../api.js';
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        successModal: false,
        warningModal: false,
        successMessage: '换办公室成功',
        warningMessage: '信息不完整，请检查填写信息',
        radios: '1',
        radio: '1',
        radio3: '1',
        radio4: '1',
...
//		      this.$set(this.imgShow,k,false)
    ...
    computed: {
      firstmoney: function() {
        ...
        //联合办公计算方式
        let q = 0;
        let fy = (vm.housetderta.roomRent * parseFloat((days-daym)/days).toFixed(10)) * (vm.discount / 100) * (parseFloat(vm.leaseDiscount)/100);//第一个月房费计算方式
        ...
        if(this.value2 == '月付') {
          ...
          if(datms.getDate() == 1){
            ...
            }else if(vm.month >= 6 && vm.month <= 12){
    ...
    methods: {
      ...
      SigController2(){
        ...
        if(!this.cyclePayType){
          ...
          axios.post(OfficeContractSign200279,param).then(res =>{
            // console.log(res);
            if(res.status == 200 && res.data.code == 10000){
            ...
            .catch(error=>{
              vm.warningMessage = '信息不完整,请检查信息填写';
              vm.warningModal = true;
