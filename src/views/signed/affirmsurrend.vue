	...
	<div id="addirmsurrends">
		...
		<div class="right-content" id="right-content">
			...
			<div class="wordbench-box">
		    	...
		    	<div id="addirmsurrend">
		    		<div class="addirmsurrend1">
		    			<Icon type="android-home"></Icon><p>{{ThrowLease.isOffice | isOffice}}{{ThrowLease.roomInfo}}</p>
		    			<ul>
		    		...
		    		<div class="addirmsurrend2">
		    			<div>
		    				...
		    				<ul>
		    					<li><span>押金：{{ThrowLease.deposit | des}}元</span></li>
...
<script>
    ...
    export default {
    	...
    	computed:{
    		refundableWaterEnergyMoney:function(){
    			if(this.ThrowLease){
    				this.money = this.ThrowLease.waterPrice * this.ThrowLease.waterCount + this.ThrowLease.energyPrice * this.ThrowLease.energyCount;
    				return this.money;
    			}
    			
    		},
    		refundableMaterialsMoney:function(){
				let q = 0;
    		...
    		sumPrice:function(){
    			this.zonmoney = this.ThrowLease.deposit + this.money -  this.money2 -  this.money3 -  this.money4;
    	...
    	methods:{
			...
			datas(){
				axios.post(hostwLeaseContr,
					...
					if(res.status == 200 && res.data.code == 10000){
						...
						for(let i = 0;i<this.refund.length;i++){
							this.$set(this.refund[i],"price",'0.00');
						}
			...
			promise(){
				...
				for(let i = 0;i<this.refund.length;i++){
					this.refundableMaterialsInfo.push({materialName:this.refund[i].materialName,count:this.refund[i].count,costAmount:this.refund[i].costAmount});
				}
				param.append("refundableMaterialsInfo",JSON.stringify(this.refundableMaterialsInfo));
				for(let i = 0;i<this.OtherInfo.length;i++){
					this.refundableOtherInfo.push({costName:this.OtherInfo[i].costName,costAmount:this.OtherInfo[i].costAmount});
				}
